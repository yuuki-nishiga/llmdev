...
from original.graph import get_bot_response, get_messages_list, memory, set_role, role_settings
...
def set_role_route():
    ...
    role_message = request.form.get('role_setting', '').strip()

    if role_message:
        set_role(role_message, memory, session['thread_id'])
...
def clear():
    ...
    session.pop('role_setting', None)

    thread_id = session.get('thread_id')
    if thread_id in role_settings:
        del role_settings[thread_id]
